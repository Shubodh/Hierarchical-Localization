...
def visualize_loc_kp(results, image_dir, sfm_model=None, top_k_db=2,
                  selected=[], n=1, seed=0, prefix=None, dpi=75):
    assert image_dir.exists()
...
#                    inliers_dbs[db_idx].append(inl)
            ...
            #print(loc['indices_db'][1000:1090])
            #sys.exit()
            counts = np.array([
                np.sum(loc['indices_db'] == i) for i in range(n)])
            #counts = np.array([
            #    np.sum(loc['indices_db'][inliers] == i) for i in range(n)])
